...
s32 get_overriding_player_anim(s32 anim) {
    ...
    if (playerStatus->animFlags & PA_FLAG_USING_PEACH_PHYSICS && !(playerStatus->animFlags & PA_FLAG_INVISIBLE)) {
        s32 sprIndex = (anim >> 16) & 0xFF;
        s32 animIndex = anim & 0xFFFF;

        if (sprIndex >= SPR_Mario1 && sprIndex <= SPR_MarioW3) {
            if (animIndex == 1) return ANIM_Peach1_Walk;
            if (animIndex == 2) return ANIM_Peach1_Run;
            if (animIndex == 3) return ANIM_Peach1_Jump;
            if (animIndex == 4) return ANIM_Peach1_Fall;
            if (animIndex == 5) return ANIM_Peach1_Walk; // Landing
        }

        if (sprIndex == SPR_MarioW1 && animIndex >= 6 && animIndex <= 0x11) {
            return ANIM_Peach2_OpenParasol;
        }
    }
