...
void phys_peach_update(void) {
    ...
    do {
        ...
        if (!(gPlayerStatus.flags & PS_FLAG_PAUSED)) {
            if (gPlayerStatus.actionState == ACTION_STATE_IDLE ||
        ...
        if (action->flag ||
            gPlayerStatus.actionState == ACTION_STATE_JUMP ||
            gPlayerStatus.actionState == ACTION_STATE_HAMMER ||
            gPlayerStatus.actionState == ACTION_STATE_FALLING ||
            gPlayerStatus.actionState == ACTION_STATE_LAND)
        {
...
b32 check_input_hammer(void) {
    ...
    if (playerStatus->pressedButtons & BUTTON_B) {
        ...
        if (!(gGameStatusPtr->peachFlags & PEACH_FLAG_IS_PEACH) && playerData->hammerLevel == -1) {
            return false;
        }
